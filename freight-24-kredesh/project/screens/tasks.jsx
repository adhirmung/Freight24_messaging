// Tasks — instruction list, modeled on the ETA dashboard
  ...
  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0, background: 'var(--bg-0)' }}>
      ...
      <div style={{
        ...
        <div style={{ flex: 1 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <Pill tone="green"><span className="live-dot" /> Auto‑extracted</Pill>
          </div>
      ...
      <div style={{ flex: 1, overflow: 'auto', padding: '18px 22px' }}>
        {tab === 'today' && <TodayMiniStats tasks={tasks} />}
        <InstructionList items={visible} setRoute={setRoute} onToggle={toggle} />
        {visible.length === 0 && (
...
function TodayMiniStats({ tasks }) {
  const t = {
    high: tasks.filter(x => x.status === 'pending' && x.priority === 'high').length,
    me:   tasks.filter(x => x.status === 'pending' && x.assignee === 'me').length,
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12, marginBottom: 18, maxWidth: 720 }}>
      <MiniBox label="High priority" value={t.high} tone="warn" />
      <MiniBox label="Assigned to me" value={t.me} tone="green" />
      <MiniBox label="Overdue" value={t.overdue} tone="bad" />
    </div>
  );
}
...
function InstructionList({ items, setRoute, onToggle }) {
  if (items.length === 0) return null;
  return (
    <div className="card" style={{ overflow: 'hidden' }}>
      {items.map((t, idx) => (
        <InstructionRow key={t.id} t={t} setRoute={setRoute} onToggle={onToggle} last={idx === items.length - 1} />
      ))}
    </div>
  );
}

function InstructionRow({ t, setRoute, onToggle, last }) {
  const D = window.KredeshData;
  const assignee = D.byId(D.users, t.assignee);
  const isMine = t.assignee === 'me';

  // Extract a clean "when" string for the time column
    ...
    <div style={{
      ...
      <div style={{ minWidth: 0 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginTop: 5, flexWrap: 'wrap' }}>
          ...
          {completed && t.completedAt && (
            <span className="mono" style={{ fontSize: 10.5, color: '#6FE3C2' }}>
              ✓ {t.completedAt}
            </span>
          )}
        </div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, minWidth: 0 }}>
        ...
        <div style={{ minWidth: 0 }}>
          ...
          <div className="mono" style={{ fontSize: 10, color: priorityColor, marginTop: 1 }}>
