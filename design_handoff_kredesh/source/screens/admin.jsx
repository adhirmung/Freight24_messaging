      ...
      .then(({ data }) => {
        if (data && data.length > 0) {
          setEmployees(data.map(p => ({
            ...
            role: p.role, status: 'active', lastActive: '—',
            joined: new Date(p.created_at).toLocaleDateString('en-ZA', { month: 'short', year: 'numeric' }),
          })));
        } else {
          // Fallback to seed data while DB is empty
  ...
  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0, background: 'var(--bg-0)' }}>
      ...
      <div style={{
        ...
        <Btn primary icon={<Icons.plus size={14} stroke="#0B141A" sw={2.4}/>} onClick={() => setShowInvite(true)}>
          Invite user
        </Btn>
      </div>
      ...
      <div style={{ flex: 1, overflow: 'auto', padding: '20px 22px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12, marginBottom: 18, maxWidth: 720 }}>
          <AdminStat label="Active"          value={counts.active}    tone="green" />
          <AdminStat label="Pending invites" value={counts.invited}   tone="warn" />
          <AdminStat label="Suspended"       value={counts.suspended} tone="bad" />
        </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12 }}>
            ...
            {[
              ...
              return (
                <button key={t.id} onClick={() => setTab(t.id)} style={{
                  ...
                  {t.label} <span className="mono" style={{ fontSize: 10, color: 'var(--ink-3)', marginLeft: 4 }}>{t.count}</span>
                </button>
              );
            })}
          </div>
          <div style={{
            ...
            background: 'var(--bg-1)', border: '1px solid var(--line)', borderRadius: 8, minWidth: 260,
          }}>
        ...
        <div className="card" style={{ overflow: 'hidden' }}>
          ...
          {visible.map(e => (
            <div key={e.id} style={{
              ...
              <button style={{ width: 26, height: 26, display: 'grid', placeItems: 'center', background: 'transparent', border: 'none', borderRadius: 6, color: 'var(--ink-2)', cursor: 'pointer' }}>
                <Icons.more size={14} stroke="var(--ink-2)" />
              </button>
            </div>
          ))}
          {visible.length === 0 && (
      ...
      {showInvite && <AdminInviteModal onClose={() => setShowInvite(false)} onInvite={addInvite} roles={D.roles} />}
    </div>
  );
}

function adminInitialsOf(name) {
  return name.split(' ').slice(0, 2).map(s => s[0]).join('').toUpperCase();
}
function adminColorOf(id) {
...
function AdminInviteModal({ onClose, onInvite, roles }) {
  ...
  const send = () => { emailList.forEach(em => onInvite({ email: em, role })); };
    ...
    <div onClick={onClose} style={{
      ...
      <div onClick={e => e.stopPropagation()} style={{
        ...
        <div style={{ padding: '16px 20px', borderBottom: '1px solid var(--line)', display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <button onClick={onClose} style={{ width: 26, height: 26, display: 'grid', placeItems: 'center', background: 'transparent', border: 'none', borderRadius: 6, color: 'var(--ink-2)', cursor: 'pointer', fontSize: 16 }}>✕</button>
        </div>
        <div style={{ padding: '20px 22px' }}>
          <label style={{ display: 'block', marginBottom: 14 }}>
            ...
            <textarea value={emails} onChange={e => setEmails(e.target.value)} rows={3}
              ...
              style={{ width: '100%', padding: '10px 12px', background: 'var(--bg-2)', border: '1px solid var(--line-2)', borderRadius: 9, color: 'var(--ink-0)', fontSize: 13, fontFamily: 'var(--mono)', outline: 'none', resize: 'vertical' }} />
          ...
          <label style={{ display: 'block' }}>
            ...
            <select value={role} onChange={e => setRole(e.target.value)} style={{ width: '100%', padding: '10px 12px', background: 'var(--bg-2)', border: '1px solid var(--line-2)', borderRadius: 9, color: 'var(--ink-0)', fontSize: 13, outline: 'none' }}>
