// ETA dashboard — chronological view of arrivals and loadouts
    ...
    <div style={{ display: 'flex', flex: 1, minWidth: 0 }}>
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0, background: 'var(--bg-0)' }}>
        ...
        <div style={{
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{ fontSize: 11.5, color: 'var(--ink-3)', marginTop: 3 }}>
              Auto-extracted from chats · {D.etas.length} scheduled across the week
            </div>
          </div>
          <div style={{
        ...
        <div style={{ flex: 1, overflow: 'auto', padding: '18px 22px' }}>
          ...
          {tab === 'today' ? (
            <EtaTodayTimeline items={visible} setRoute={setRoute} />
          ) : (
            <EtaSimpleList items={visible} setRoute={setRoute} />
          )}
...
function EtaTodayTimeline({ items, setRoute }) {
...
function EtaSimpleList({ items, setRoute }) {
  const byWhen = {};
  ...
  const labelOf = w => ({ tomorrow: 'Tomorrow', thursday: 'Thursday', friday: 'Friday', 'fri 9': 'Fri 9 Jan' })[w] || w;
